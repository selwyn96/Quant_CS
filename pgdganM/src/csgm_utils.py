...
def get_estimator(hparams, model_type):
    if hparams.dataset == 'mnist':
        ...
        if model_type == 'lasso-dct':
            estimator = csgm_estimator.lasso_dct_estimator(hparams)
        elif model_type == 'lasso-wavelet':
            estimator = csgm_estimator.lasso_wavelet_estimator(hparams)
        elif model_type == 'lasso-wavelet-ycbcr':
            estimator = csgm_estimator.lasso_wavelet_ycbcr_estimator(hparams)
        elif model_type == 'k-sparse-wavelet':
            estimator = csgm_estimator.k_sparse_wavelet_estimator(hparams)
        elif model_type == 'dcgan':
            estimator = csgm_estimator.dcgan_estimator(hparams)
        else:
            raise NotImplementedError
    return estimator
...
def get_A(hparams):
    if hparams.measurement_type == 'gaussian':
       A = (1.0 / np.sqrt(hparams.num_measurements)) * np.random.randn(hparams.n_input,
                                                                                hparams.num_measurements)
...
def get_A(hparams):
 ...
 #   lists=np.random.randint(low=0, high=hparams.n_input-1, size=hparams.num_measurements)
    A_outer=np.random.binomial(1,0.5,(hparams.num_measurements,hparams.n_input))
    np.place(A_outer, A_outer==0, [-1])
  #  A_outer=A[lists]
    A_outer=A_outer.T
    A_outer =  (1.0 / (np.sqrt(hparams.num_measurements))) * A_outer
   # print A_outer.shape
  #  A_outer =  (1.0 / (np.sqrt(hparams.num_measurements))) * A_outer
