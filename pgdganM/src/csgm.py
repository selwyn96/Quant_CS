...
def main(hparams):
    ...
    hparams.n_input = 4096 #np.prod(hparams.image_shape)
    csgm_utils.set_num_measurements(hparams)
    csgm_utils.print_hparams(hparams)

    # get inputs
    xs_dict = model_input(hparams)

    estimators = csgm_utils.get_estimators(hparams)
    csgm_utils.setup_checkpointing(hparams)
    measurement_losses, l2_losses = csgm_utils.load_checkpoints(hparams)
    ...
    for key, x in xs_dict.iteritems():
        if not hparams.not_lazy:
            ...
            save_paths = csgm_utils.get_save_paths(hparams, key)
      ...
      #  noise_batch = hparams.noise_std * np.random.randn(hparams.batch_size, hparams.num_measurements)
        ...
        for model_type in hparams.model_types:
            ...
            for i, key in enumerate(x_batch_dict.keys()):
                ...
                measurement_losses[model_type][key] = csgm_utils.get_measurement_loss(x_hat, A, y)
                l2_losses[model_type][key] = csgm_utils.get_l2_loss(x_hat, x)

        print 'Processed upto image {0} / {1}'.format(key+1, len(xs_dict))

        # Checkpointing
        if (hparams.save_images) and ((key+1) % hparams.checkpoint_iter == 0):
            csgm_utils.checkpoint(x_hats_dict, measurement_losses, l2_losses, save_image, hparams)
            x_hats_dict = {model_type : {} for model_type in hparams.model_types}
    ...
    if hparams.image_matrix > 0:
        csgm_utils.image_matrix(xs_dict, x_hats_dict, view_image, hparams)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--num-measurements', type=int, default=1000, help='number of gaussian measurements')
    ...
    PARSER.add_argument('--mloss1_weight', type=float, default=0.1, help='L1 measurement loss weight')
    PARSER.add_argument('--mloss2_weight', type=float, default=1.0, help='L2 measurement loss weight')
    PARSER.add_argument('--zprior_weight', type=float, default=0.01, help='weight on z prior')
    PARSER.add_argument('--dloss1_weight', type=float, default=0.1, help='-log(D(G(z))')
    PARSER.add_argument('--dloss2_weight', type=float, default=0.1, help='log(1-D(G(z))')
